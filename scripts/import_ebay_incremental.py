...
from catalog_rules import is_relevant_product, normalize_product_payload, clean_title
...
USER_AGENT = 'CampMateIncrementalImporter/1.1'
...
def first_image(item: Dict[str, Any]) -> str:
    ...
    if image and str(image).startswith('http'):
        return str(image)
    return CATEGORY_FALLBACKS.get(item.get('_campmate_category', 'tents'), CATEGORY_FALLBACKS['tents'])
...
def should_skip(item: Dict[str, Any], brand: str, category: str, exclude_terms: List[str]) -> bool:
    probe = {
        ...
        'summary': ' '.join(filter(None, [item.get('shortDescription'), item.get('subtitle')])),
        'brand': brand,
    }
    if not is_relevant_product(normalize_product_payload(probe, category), category):
        return True
    text = f"{item.get('title','')} {item.get('shortDescription','')} {item.get('subtitle','')}".lower()
...
def import_one(item: Dict[str, Any], *, category: str, category_name: str, fallback_brand: str, existing_slugs: set) -> Dict[str, Any]:
    ...
    title = clean_title(str(item.get('title') or '').strip() or f'{brand} {category_name}', brand)
    ...
    return normalize_product_payload(product, category)


def _dedupe_key(item: Dict[str, Any]) -> str:
    return str(item.get('sourceItemId') or item.get('sourceLegacyItemId') or item.get('slug'))
...
def merge_products(existing: List[Dict[str, Any]], new_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    seen_title_image = set()
    for item in merged:
        category = item.get('category') or 'tents'
        item = normalize_product_payload(item, category)
        if not is_relevant_product(item, category):
            continue
        sig = (slugify(item.get('brand','')), slugify(item.get('name','')), str(item.get('image') or item.get('ebayImage') or ''))
        if sig in seen_title_image:
            continue
        seen_title_image.add(sig)
        cleaned.append(item)
    return cleaned
...
def save_and_rebuild(products: List[Dict[str, Any]]):
    ...
    completed = subprocess.run([sys.executable, str(BUILD_SCRIPT)], cwd=str(ROOT), capture_output=True, text=True, check=False)
    ...
    for brand in brands:
        ...
        while stats[brand] < needed and tries < 20:
            ...
            for summary in items:
                ...
                if not is_relevant_product(product, category):
                    continue
...
def main():
    ...
    parser.add_argument('--per-request', '--page-size', dest='per_request', type=int, default=100, help='eBay search page size (max 200)')
