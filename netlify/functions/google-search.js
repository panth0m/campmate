exports.handler = async function (event) {
  try {
    ...
    if (!apiKey || !cx) {
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'Missing Google search environment variables' }),
      };
    }
    ...
    if (!response.ok) {
      return {
        ...
        body: JSON.stringify({ error: 'Google search request failed', details: data }),
      };
    }
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'Google search failed', details: error.message }),
    };
  }
};
